...
class ITSEC_Malware_Scanner {
	...
	protected static function get_scan_results() {
		...
		if (
			...
			return new WP_Error( 'itsec-malware-scanner-malformed-wp-remote-get-response', __( 'The response from the wp_remote_get function was malformed. This could indicate an issue with WordPress.', 'it-l10n-ithemes-security-pro' ) );
		}
		
		if ( 'application/json' !== $response['headers']['content-type'] ) {
			return new WP_Error( 'itsec-malware-scanner-invalid-content-type-in-scan-response', sprintf( __( 'The Sucuri server returned an invalid content type. A content type of <code>%s</code> was received when a content type of <code>application/json</code> was expected. This could indicate a temporary issue with the Sucuri servers.', 'it-l10n-ithemes-security-pro' ), esc_html( $response['headers']['content-type'] ) ) );
		}
		...
		if ( ! is_array( $body ) ) {
			if ( 'ERROR' === substr( $response['body'], 0, 5 ) ) {
				return new WP_Error( 'itsec-malware-scanner-error-received', sprintf( __( 'The scan did not complete successfully. Sucuri sent the following error: %s', 'it-l10n-ithemes-security-pro' ), $response['body'] ) );
			}
			
			return new WP_Error( 'itsec-malware-scanner-unknown-scan-error', sprintf( __( 'An unknown error prevented the scan from completing successfully. The Sucuri server responded with a <code>%s</code> error code.', 'it-l10n-ithemes-security-pro' ), $response['response']['code'] ) );
		}
