	...
	class CodespacingProgressMap{
		...
		function cspm_scripts(){		
			...
			if($this->combine_files == "combine"){
				...
				wp_register_script('cspm_google_maps_api', ''.$this->use_ssl.'://maps.google.com/maps/api/js?v=3.exp&sensor=true&language='.$this->map_language.'&libraries=geometry,places', array( 'jquery' ), false, true);
				wp_register_script('cspm_frontend_scripts_js', $this->plugin_url .'js/min/cspm_frontend_scripts.min.js', array( 'jquery' ), false, true);
				...
				wp_register_script('cspm_jqueryui_js', $this->plugin_url .'js/min/jquery-ui-1.10.3.custom.min.js', array( 'jquery' ), false, true);
				wp_register_script('cspm_jqueryui_effect_js', $this->plugin_url .'js/min/jquery.ui.effect.min.js', array( 'jquery' ), false, true);
				wp_register_script('cspm_jqueryui_effect_drop_js', $this->plugin_url .'js/min/jquery.ui.effect-drop.min.js', array( 'jquery' ), false, true);
				wp_register_script('cspm_jqueryui_effect_slide_js', $this->plugin_url .'js/min/jquery.ui.effect-slide.min.js', array( 'jquery' ), false, true);
				wp_register_script('cspm_livequery_js', $this->plugin_url .'js/min/jquery.livequery.min.js', array( 'jquery' ), false, true);
				wp_register_script('cspm_easing', $this->plugin_url .'js/min/jquery.easing.1.3.min.js', array( 'jquery' ), false, true);
				wp_register_script('cspm_google_maps_api', ''.$this->use_ssl.'://maps.google.com/maps/api/js?v=3.exp&sensor=true&language='.$this->map_language.'&libraries=geometry,places', array( 'jquery' ), false, true);
				wp_register_script('cspm_gmap3_js', $this->plugin_url .'js/min/gmap3.min.js', array( 'jquery' ), false, true);		
				wp_register_script('cspm_markerclusterer_js', $this->plugin_url .'js/min/MarkerClustererPlus.min.js', array(), false, true);
				wp_register_script('cspm_touchSwipe_js', $this->plugin_url .'js/min/jquery.touchSwipe.min.js', array( 'jquery' ), false, true);		
				wp_register_script('cspm_jcarousel_js', $this->plugin_url .'js/min/jquery.jcarousel.min.js', array( 'jquery' ), false, true);
				wp_register_script('cspm_mCustomScrollbar_js', $this->plugin_url .'js/min/jquery.mCustomScrollbar.min.js', array( 'jquery' ), false, true);		
				wp_register_script('cspm_icheck_js', $this->plugin_url .'js/min/jquery.icheck.min.js?v=0.9.1', array( 'jquery' ), false, true);
				wp_register_script('cspm_nprogress_js', $this->plugin_url .'js/min/nprogress.min.js', array( 'jquery' ), false, true);
				wp_register_script('cspm_rangeSlider_js', $this->plugin_url .'js/ion.rangeSlider.js', array( 'jquery' ), false, true);
				wp_register_script('cspm_progress_map_js', $this->plugin_url .'js/progress_map.js', array( 'jquery' ), false, true);					
		...
		* @since 2.0 */
		...
		function cspm_main_map_shortcode($atts){
			...
			<script type="text/javascript">
				...
				jQuery(document).ready(function($) { 		
					...
					if(typeof _CSPM_DONE === 'undefined' || _CSPM_DONE[map_id] === true) return;
											...
											callback: function(markers){
												...
												if(!light_map){
													...
													for(var i = 0; i < markers.length; i++){	
														...
														var lat = marker_position.lat();//marker_position[0];
														var lng = marker_position.lng();//marker_position[1];											
			...
			$output .= '<div class="cspm_infobox_content_container '.$status.' infobox_'.$map_id.' '.$infobox_type.'" data-map-id="'.$map_id.'" data-post-id="'.$post_id.'" data-show-carousel="'.$carousel.'">';
				...
				if($infobox_type == 'square_bubble' || $infobox_type == 'rounded_bubble'){
					...
					$output .= '<div class="cspm_infobox_content">';
						...
						$output .= '<a class="btn map" href="https://www.google.com/maps/place/'. $location .'" target="_blank">Get Directions</a>';
					$output .= '</div>';
					$output .= '<div style="clear:both"></div>';
		...
		function cspm_after_settings(){
			...
			echo '<div class="codespacing_copyright">&copy; All rights reserved Codespacing. Progress Map 2.7.5</div>';
